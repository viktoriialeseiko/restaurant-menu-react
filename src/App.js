...
class App extends Component {
  ...
  render() {
    const filteredMenu = this.state.menu.filter(menu => {
      if(this.state.activeCat === "all") {
        return true;
      }
      return menu.category === this.state.activeCat;
    });
    return (
      <div>
        <div className="categories">
          {this.state.categories.map(cat => {
            ...
            return <button className={cl} onClick={() => this.onCatClick(cat)}>{cat}</button>
          })}
        </div>
        <div>Cart: ${this.state.totalPrice}</div>
        <ul>
          {filteredMenu.map(m => {
            return <li>
              {m.title} - {m.price}
              <button onClick={() => this.addToCart(m.price)}>add to cart</button>
            </li>
          })}
        </ul>
      </div>
